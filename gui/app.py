...
# Allow running this file directly (`python gui/app.py`) by ensuring the
# package root (midum_pkg, the parent of this gui/ folder) is on sys.path.
# Without this, Python only sees the gui/ folder itself on sys.path when the
# script is launched directly, so the top-level "gui" package can't be found.
